#!/usr/bin/env python3
"""Run world +Y pitch while translating along world +X.

Default task:
    pitch: 0 -> +360 deg about world +Y
    x:     0 -> 1.2 m at 1/300 m per degree
    z:     0.35 m fixed

Only the task path is new. UnifiedContactPlanner search/recovery semantics are
unchanged.
"""
...
from lily_contact_planner.tasks import Pitch45ThenRoll45Task, PitchForwardTask
...
def _build_height_consistent_initial_q(kin, task):
    """Build the same ground-safe 0.35 m initial posture used by the hard task."""
    ...
    for leg in range(kin.n_legs):
        ...
        if not safe:
            raise RuntimeError(
                "cannot construct a ground-safe initial IK posture for lower leg {} "
                "at target {}".format(leg, target.tolist())
            )
    ...
    if geom_min < -1e-8:
        raise RuntimeError(
            "constructed initial posture is globally infeasible: "
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=360.0,
        help="Total world +Y pitch angle in degrees.",
    )
    parser.add_argument(
        "--forward-m-per-deg",
        type=float,
        default=1.0 / 300.0,
        help="World +X translation per degree of scalar progress.",
    )
    parser.add_argument(
        "--body-height-m",
        type=float,
        default=0.35,
        help="Fixed body-center world z height.",
    )
    parser.add_argument(
        "--output",
        default="pitch360_forward_result.json",
        help="Final JSON output path.",
    )
    parser.add_argument(
        "--checkpoint",
        default="pitch360_forward_checkpoint.json",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Diagnostic stack dump interval. 0 disables periodic dumps.",
    )
    parser.add_argument(
        "--probe-init-only",
        action="store_true",
    ...
    if args.pitch_deg <= 0.0:
        raise ValueError("--pitch-deg must be > 0")
    if args.body_height_m <= 0.0:
        raise ValueError("--body-height-m must be > 0")
        ...
        json.dumps(
            {
                ...
                "final_translation_x_m": task.forward_m_per_deg * task.pitch_deg,
